...
try:
    from src.preprocessing import SEED, TARGET, prepare_target
except (ImportError, ModuleNotFoundError):
    from preprocessing import SEED, TARGET, prepare_target

def evaluate(name, y_true, y_pred, results=None):
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    mae = mean_absolute_error(y_true, y_pred)
    r2 = r2_score(y_true, y_pred)
    mape = np.mean(np.abs((np.expm1(y_true) - np.expm1(y_pred)) / (np.expm1(y_true) + 1e-9))) * 100
...
def train_final_stacking(X, y):
    ...
    stack = StackingRegressor(
        estimators=[
            ...
            ('xgb', XGBRegressor(n_estimators=300, learning_rate=0.05, max_depth=6, subsample=0.8, random_state=SEED, verbosity=0)),
            ('lgbm', LGBMRegressor(n_estimators=300, learning_rate=0.05, num_leaves=63, subsample=0.8, random_state=SEED, verbose=-1))
        ],
...
def run_cp2(data_dir: str = "data/processed", models_dir: str = "models"):
    ...
    print(f"RMSE: {f['rmse']:.4f} | MAE: {f['mae']:.4f} | R²: {f['r2']:.4f} | MAPE: {f['mape']:.1f}%")
